...
/**
  * executeInstruction -function to execute opcode function
  * @stack: my stack
  * @instructions: list of instructions
  * @numInstructions: number of instructions
  * @opcode: code in file
  * @line_number: code lin number in file
  * @file_line: line of code in file
  */

void executeInstruction(stack_t **stack, instruction_t *instructions,
		size_t numInstructions, char *opcode,
		unsigned int line_number, char *file_line)
{
	size_t i;

	char *code = strtok(opcode, " $\t\n");
...
void get_instruction(stack_t **stack, unsigned int line_number,
		char *file_line)
{
	char opcode[20];
	...
	executeInstruction(stack, instructions, numInstructions,
			opcode, line_number, file_line);
}
